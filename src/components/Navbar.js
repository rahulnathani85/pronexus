          ...
          {NAV_ITEMS.map(function(item) {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={'px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 no-underline whitespace-nowrap ' + cls}>
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="flex gap-2 items-center">
          {user ? (
            <>
              ...
              <button
                className="bg-transparent text-red-600 border border-red-200 py-1.5 px-4 text-[13px] rounded-lg font-semibold cursor-pointer hover:bg-red-50 transition-all"
